...
(function(){
  ...
  function base(){
    return location.pathname.includes('/swavli/') ? '/swavli/' : '/';
  }
  ...
  function clean(){
    ...
    if(p.endsWith('/register.html')){
      history.replaceState(null, '', p.replace(/register\.html$/, 'register/') + q);
    }

    if(p.endsWith('/teachers.html')){
      history.replaceState(null, '', p.replace(/teachers\.html$/, 'teachers/') + q);
    }

    if(p.endsWith('/teacher.html')){
      history.replaceState(null, '', p.replace(/teacher\.html$/, 'teacher/') + q);
    }
  }
  ...
  function optionHtml(items, type){
    return items.map(([value, label], idx) => {
      return `<div class="dd-opt${idx === 0 ? ' active' : ''}" data-val="${escapeAttr(value)}" onclick="pickOptionSafe('${type}', this, event)">${label}</div>`;
    }).join('');
  }
    ...
    try{
      if(typeof CATEGORIES !== 'undefined' && typeof CAT_LABELS !== 'undefined'){
        return CATEGORIES.map((value, i) => [
          String(value || '').replace(/^[^\wა-ჰ]+/, '').trim(),
          CAT_LABELS[i] || value || 'ყველა კატეგორია'
        ]);
      }
    }catch(e){}
  ...
  function buildRegions(){
    try{
      if(typeof REGIONS !== 'undefined'){
        return REGIONS.map((value, i) => [
          value || '',
          i === 0 ? 'ყველა რეგიონი' : '📍 ' + value
        ]);
      }
    }catch(e){}
  ...
  function fillDropdowns(){
    ...
    if(cat && !cat.dataset.fixed){
      cat.innerHTML = optionHtml(buildCats(), 'cat');
      cat.dataset.fixed = '1';
    }

    if(reg && !reg.dataset.fixed){
      reg.innerHTML = optionHtml(buildRegions(), 'reg');
      reg.dataset.fixed = '1';
    }

    if(fmt){
      fmt.innerHTML = optionHtml(FALLBACK_FORMATS, 'fmt');
      fmt.dataset.fixed = '1';
    }
  }
  ...
  window.goSearch = function(cat){
    location.href = base() + 'teachers/?cat=' + encodeURIComponent(cat || '');
  };
  ...
  window.addEventListener('resize', () => {
    setTimeout(setSearchOverflowVisible, 30);
  });
